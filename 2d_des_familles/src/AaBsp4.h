    ...
    void for_each_candidate_at( TF p0x, TF p0y, SI i0, MayCut &&may_cut, CutWith &&cut_with,
        ...
        if constexpr ( Lazy ) {
            ...
            while ( top > 0 ) {
                ...
                if ( ! may_cut( nd.lo[ 0 ][ s ], nd.lo[ 1 ][ s ], nd.hi[ 0 ][ s ], nd.hi[ 1 ][ s ],
                                nd.wm[ s ] ) )
                    continue;
        ...
        while ( top > 0 ) {
            ...
            SI nc = 0;
            for ( SI i = 0; i < nd.nb; ++i ) {
                if ( ! may_cut( nd.lo[ 0 ][ i ], nd.lo[ 1 ][ i ], nd.hi[ 0 ][ i ], nd.hi[ 1 ][ i ],
                                nd.wm[ i ] ) )
                    continue;
                key[ nc ] = near( nd, i );
                cs[ nc ] = i;
...
private:
    ...
    void set_child( const TF *X, const TF *Y, const TF *W, SI me, SI i, SI beg, SI end,
        ...
        nodes[ me ].end[ i ] = end;
        if ( W )
            nodes[ me ].wm[ i ] = weight_majorant( beg, end, [ & ]( SI k, TF &x, TF &y, TF &w ) {
                const SI j = order[ k ];
                x = X[ j ]; y = Y[ j ]; w = W[ j ];
            } );
    }
