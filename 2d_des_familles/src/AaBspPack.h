...
struct AaBspPack {
    ...
    void for_each_candidate( SI k0, MayCut &&may_cut, CutWith &&cut_with, Reach2 &&reach2 ) const {
        ...
        const TF p0x = full.px[ k0 ], p0y = full.py[ k0 ], w0 = full.seed_w( k0 );
        ...
        for ( SI q = loff[ u ]; q < loff[ u + 1 ]; ++q ) {
            ...
            if ( ldq[ q ] > seuil )
                return;
            if ( ! full.for_each_candidate_from( pk[ lval[ q ] ].node, p0x, p0y, i0,
    ...
    void build( const TF *X, const TF *Y, const TF *W, SI n, SI leaf ) {
        ...
        for ( SI u = 0; u < m; ++u ) {
            ...
            for ( SI k = b.beg; k < b.end; ++k ) {
                const TF ex = full.px[ k ] - cx, ey = full.py[ k ] - cy;
        ...
        for ( SI u = 0; u < m; ++u ) {
            for ( SI k = pk[ u ].beg; k < pk[ u ].end; ++k ) { qx[ u ] += full.px[ k ];
                                                              qy[ u ] += full.py[ k ]; }
        ...
        parallel_for( n, nth, Split::blocks, false, [ & ]( SI k, int ) {
            const TF mx = full.px[ k ], my = full.py[ k ], mw = full.seed_w( k );
            Cell ce;
            PowerDiagram<Cell, OneSeed, true, false, false>{
                OneSeed{ sub, mx, my, mw, full.order[ k ] } }.make_cell( ce, 0 );
        ...
        for ( SI u = 0; u < m; ++u ) {
            Cell c;
            PowerDiagram<Cell, OneSeed, true, false, false>{
                OneSeed{ sub, qx[ u ], qy[ u ], delta[ u ], SI( -1 ) } }.make_cell( c, 0 );
