...
template<int MaxNv>
struct CellSoAT {
    static constexpr int dim = 2;                       ///< la dimension, lue par `PowerDiagram`
    ...
    /// LES DEUX FACADES GENERIQUES, celles par lesquelles `PowerDiagram` parle a la cellule quelle
    /// que soit la dimension. Elles ne font que renommer : le corps 2D ci-dessous n'a pas bouge d'une
    /// ligne, et les surcharges scalaires restent pour tout le code de banc qui est 2D par nature
    /// (les fronts, les enclos, les traces).
    Vec<2> vertex( SI i ) const { return { vx[ i ], vy[ i ] }; }

    CutResult cut( Vec<2> d, TF off, SI cut_id ) { return cut( d[ 0 ], d[ 1 ], off, cut_id ); }

    void bounds( TF *lo, TF *hi ) const { bounds( lo[ 0 ], lo[ 1 ], hi[ 0 ], hi[ 1 ] ); }

    /// LES FACETTES : `f( coupe, mesure )` pour chaque cote porte par un VOISIN (les cotes du
    /// domaine sont sautes). En 2D c'est l'invariant qui la donne -- la coupe `i` porte l'arete
    /// `[ v_i, v_i+1 ]` -- alors qu'en 3D il faut retrouver un cycle ; c'est la seule porte par
    /// laquelle Newton lit la geometrie, et c'est ce qui lui permet d'ignorer la dimension.
    template<class F>
    void for_each_facet( F &&f ) const {
        for ( SI v = 0; v < nb; ++v ) {
            if ( cid[ v ] < 0 )
                continue;
            const SI u = v + 1 < nb ? v + 1 : 0;
            const TF ex = vx[ u ] - vx[ v ], ey = vy[ u ] - vy[ v ];
            f( cid[ v ], std::sqrt( ex * ex + ey * ey ) );
        }
    }

    /// LES DEUX D'UN COUP. En 2D il n'y a rien a mutualiser -- l'aire est un lacet, les facettes
    /// sont les cotes, et les deux passes sont aussi courtes l'une que l'autre. En 3D elles
    /// sortent au contraire de la MEME accumulation de faces, et Newton veut les deux a chaque
    /// cellule. La porte est donc la meme dans les deux dimensions, et c'est la 3D qui y gagne.
    template<class F>
