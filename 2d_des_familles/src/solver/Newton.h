...
template<int D>
void refresh_weights( AaBspT<D> &tr, const TF *W, int nb_threads, Split split, bool pin ) {
    ...
    parallel_for( SI( tr.nodes.size() ), nb_threads, split, pin, [ & ]( SI i, int ) {
        typename AaBspT<D>::Node &nd = tr.nodes[ i ];
        nd.wm = weight_majorant<D>( nd.beg, nd.end, [ & ]( SI k, Vec<D> &q, TF &w ) {
            for ( int d = 0; d < D; ++d ) q[ d ] = tr.p[ d ][ k ];
            w = tr.pw[ k ];
        } );
    } );
}
...
template<int D> const AaBspT<D> &base( const AaBspT<D> &t ) { return t; }
...
template<int D>
TF psi_min( const AaBspT<D> &tr, Vec<D> x, SI hors = -1 ) {
    TF best = 1e300;
    tr.for_each_candidate_at( x, hors,
        [ & ]( Vec<D> lo, Vec<D> hi, const WMajT<D> &wm ) {
            TF dd = 0, wmax = wm.b;
            for ( int d = 0; d < D; ++d ) {
                const TF e = x[ d ] < lo[ d ] ? lo[ d ] - x[ d ]
                                              : ( x[ d ] > hi[ d ] ? x[ d ] - hi[ d ] : TF( 0 ) );
                dd += e * e;
                const TF s = TF( wm.a[ d ] ) * lo[ d ], t = TF( wm.a[ d ] ) * hi[ d ];
                wmax += s > t ? s : t;
            }
            return dd - wmax < best;
        },
        [ & ]( Vec<D> q, TF w, SI ) {
            const TF h = dist2( q, x ) - w;
            if ( h < best )
                best = h;
            return true;
        },
...
template<int D, class PD, class Tree>
SI rattrape_vides( PD &pd, Tree &tr, const TF *const *P, SI m, std::vector<TF> &w,
    ...
    for ( int p = 0; p < passes; ++p ) {
        ...
        parallel_for( m, th, sp, pin, [ & ]( SI i, int ) {
            ...
            Vec<D> q;
            for ( int d = 0; d < D; ++d ) q[ d ] = P[ d ][ i ];
            w2[ i ] = -psi_min<D>( base( tr ), q, i ) + marge;
        } );
        w.swap( w2 );
    }
    if ( trace ) {
...
struct Newton {
    ...
    void aires_et_aretes( PD &pd, Tree &tr, const TF *const *P, const TF *W,
                          std::vector<TF> &res, std::vector<Arete> &ar, int th, Split sp,
                          bool pin ) {
        constexpr int D = Cell::dim;
        double t0 = now();
        ...
        parallel_for( n, th, sp, pin, [ & ]( SI k, int t ) {
            ...
            // `c_ij = |facette| / ( 2 |p_i - p_j| )` -- longueur sur distance en 2D, aire sur
            // distance en 3D. La formule ne depend pas de la dimension, seule la MESURE de la
            // facette en depend, et la cellule la livre. Mesure ET facettes par le meme appel :
            // en 3D elles sortent d'une seule accumulation de faces, et les demander separement
            // la faisait deux fois.
            res[ i ] = c.measure_and_facets( [ & ]( SI j, TF mes ) {
                TF d2 = 0;
                for ( int d = 0; d < D; ++d ) {
                ...
                if ( ! ( d2 > 0 ) )
                    return;
                par[ t ].push_back( Arete{ i, j, mes / ( 2 * std::sqrt( d2 ) ) } );
            } );
        } );
        ar.clear();
        for ( auto &v : par )
    ...
    template<class Cell, class PD, class Tree>
    bool resout( PD &pd, Tree &tr, const TF *const *P, const std::vector<TF> &w_init,
        ...
        aires_et_aretes<Cell>( pd, tr, P, w.data(), a, ar, th, sp, pin );
        ...
        for ( int it = 0; it < maxit; ++it ) {
            ...
            for ( int essai = 0; essai < 60; ++essai ) {
                ...
                aires_et_aretes<Cell>( pd, tr, P, w2.data(), a2, ar2, th, sp, pin );
