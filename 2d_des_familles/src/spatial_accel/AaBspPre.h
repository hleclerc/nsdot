...
struct AaBspPreT {
    ...
    void for_each_candidate( SI k0, MayCut &&may_cut, CutWith &&cut_with, Reach2 &&reach2 ) const {
        ...
        bool alive = true;
        sub.for_each_candidate_at( p0, i0, may_cut,
            [ & ]( Vec<2> q, TF w, SI id ) { return alive = cut_with( q, w, id ); },
            reach2 );

        if constexpr ( ! SubOnly )
            if ( alive )
                rest.for_each_candidate_at( p0, i0, may_cut, cut_with, reach2 );
    }
    ...
    void build( const TF *X, const TF *Y, const TF *W, SI n, SI leaf ) {
        ...
        sub .build_sel( P, W, in .data(), SI( in .size() ), leaf );
        if ( pre_overlap )
            rest.build( X, Y, W, n, leaf );
        else
            rest.build_sel( P, W, out.data(), SI( out.size() ), leaf );
    }
};
