...
struct FrontPd {
    ...
    TF seed_x( SI k ) const { return full.p[ 0 ][ k ]; }
    TF seed_y( SI k ) const { return full.p[ 1 ][ k ]; }
    ...
    TF ecart( TF px, TF py, TF pw, SI c ) const {
        ...
        const TF kx = full.p[ 0 ][ k ], ky = full.p[ 1 ][ k ], kw = full.seed_w( k );
    ...
    void for_each_candidate( SI k0, MayCut &&may_cut, CutWith &&cut_with, Reach2 &&reach2 ) const {
        ...
        if ( qoff[ k0 ] == qoff[ k0 + 1 ] ) {
            full.for_each_candidate_at( full.seed( k0 ), i0, may_cut, cut_with, reach2 );
            return;
        }
        for ( SI u = qoff[ k0 ]; u < qoff[ k0 + 1 ]; ++u ) {
            const SI p = qval[ u ];
            const SI id = full.order[ p ];
            if ( id != i0 && ! cut_with( full.seed( p ), full.seed_w( p ), id ) )
                return;
        }
    }
    ...
    void build( const TF *X, const TF *Y, const TF *W, SI n, SI leaf ) {
        ...
        sub.build_sel( PP, W, in.data(), SI( in.size() ), leaf );
        t_arbres += now_() - tb;
...
private:
    ...
    static void refresh_maj( AaBsp &tr, const TF *W ) {
        ...
        for ( AaBsp::Node &nd : tr.nodes )
            nd.wm = weight_majorant<2>( nd.beg, nd.end, [ & ]( SI k, Vec<2> &q, TF &w ) {
                q[ 0 ] = tr.p[ 0 ][ k ]; q[ 1 ] = tr.p[ 1 ][ k ]; w = tr.pw[ k ];
            } );
    }
    ...
    void prepare( const TF *X, const TF *Y, const TF *W, SI n, int th ) {
        ...
        parallel_for( n, th, Split::blocks, false, [ & ]( SI k, int ) {
            const TF px = full.p[ 0 ][ k ], py = full.p[ 1 ][ k ], pw = full.seed_w( k );
        ...
        parallel_for( n, th, Split::blocks, false, [ & ]( SI k, int t ) {
            ...
            const TF px = full.p[ 0 ][ k ], py = full.p[ 1 ][ k ];
            std::sort( q.begin() + deb, q.end(), [ & ]( SI a, SI b ) {
                const TF ax = full.p[ 0 ][ a ] - px, ay = full.p[ 1 ][ a ] - py;
                const TF bx = full.p[ 0 ][ b ] - px, by = full.p[ 1 ][ b ] - py;
    ...
    SI localise( TF x, TF y ) const {
        TF best = 1e300;
        SI bi = -1;
        sub.for_each_candidate_at( Vec<2>{ x, y }, -1,
