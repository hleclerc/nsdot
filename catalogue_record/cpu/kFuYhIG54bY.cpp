...
struct cut_fwd_kernel {
    template<class BatchIndex, class T_cut_id, class T_direction, class T_offset, class T_cell, class T_res, class T_scratch>
    HD void operator()( BatchIndex batch_index, int thread_index, int nb_threads, T_cut_id cut_id, T_direction direction, T_offset offset, T_cell cell, T_res res, T_scratch scratch ) const {
        cell( batch_index ).cut( res( batch_index ), scratch( batch_index ), direction, offset, cut_id );
    }
};


static ffi::Error sdot_ffi_impl( ffi::BufferR1<ffi::F64> ffi_direction, ffi::BufferR0<ffi::F64> ffi_offset, ffi::BufferR2<ffi::F64> ffi_vertex_positions, ffi::BufferR2<ffi::S32> ffi_vertex_cuts, ffi::BufferR2<ffi::S32> ffi_vertex_nbrs, ffi::BufferR1<ffi::S32> ffi_cut_ids, ffi::BufferR1<ffi::S32> ffi_nb_vertices, ffi::BufferR1<ffi::S32> ffi_nb_cuts, ffi::Result<ffi::BufferR2<ffi::F64>> ffi_vertex_positions_, ffi::Result<ffi::BufferR2<ffi::S32>> ffi_vertex_cuts_, ffi::Result<ffi::BufferR2<ffi::S32>> ffi_vertex_nbrs_, ffi::Result<ffi::BufferR1<ffi::S32>> ffi_cut_ids_, ffi::Result<ffi::BufferR1<ffi::S32>> ffi_nb_vertices_, ffi::Result<ffi::BufferR1<ffi::S32>> ffi_nb_cuts_, ffi::Result<ffi::BufferR2<ffi::S32>> ffi_words, ffi::Result<ffi::BufferR1<ffi::S32>> ffi_nb_threads, ffi::Result<ffi::BufferR1<ffi::S32>> ffi_nb_words, ffi::Result<ffi::BufferR1<ffi::S32>> ffi_errors, int64_t attr_cut_id, int64_t max_ffi_nb_vertices, int64_t max_ffi_nb_cuts, int64_t max_ffi_nb_vertices_, int64_t max_ffi_nb_cuts_, int64_t max_ffi_nb_threads, int64_t max_ffi_nb_words ) {
    ...
    auto direction = tensor_view<CpuHostMemorySpace>( ffi_direction.typed_data(), tuple( Ct<SI, 3>() ), tuple( dim_axis ) );
    auto offset = tensor_view<CpuHostMemorySpace>( ffi_offset.typed_data(), tuple(  ), tuple(  ) );
    auto cell = Cell_N{
    ...
    auto cell_io = Cell_N_io{ InpList(), InpList(), InpList(), InpList(), InpList(), InpList(), InpList() };
    auto res = Cell_N{
        tensor_view<CpuHostMemorySpace>( ffi_vertex_positions_->typed_data(), tuple( SI( ffi_vertex_positions_->dimensions()[ 0 ] ), Ct<SI, 3>() ), tuple( num_vertex, dim_axis ) ),
        tensor_view<CpuHostMemorySpace>( ffi_vertex_cuts_->typed_data(), tuple( SI( ffi_vertex_cuts_->dimensions()[ 0 ] ), Ct<SI, 3>() ), tuple( num_vertex, dim_axis ) ),
        tensor_view<CpuHostMemorySpace>( ffi_vertex_nbrs_->typed_data(), tuple( SI( ffi_vertex_nbrs_->dimensions()[ 0 ] ), Ct<SI, 3>() ), tuple( num_vertex, dim_axis ) ),
        tensor_view<CpuHostMemorySpace>( ffi_cut_ids_->typed_data(), tuple( SI( ffi_cut_ids_->dimensions()[ 0 ] ) ), tuple( num_cut ) ),
        make_shape_var_view( tensor_view<CpuHostMemorySpace>( ffi_nb_vertices_->typed_data(), tuple(  ), tuple(  ) ), SI( max_ffi_nb_vertices_ ), errors, SI( 0 ) ),
        make_shape_var_view( tensor_view<CpuHostMemorySpace>( ffi_nb_cuts_->typed_data(), tuple(  ), tuple(  ) ), SI( max_ffi_nb_cuts_ ), errors, SI( 1 ) ),
        Ct<SI, 3>{}
    };
    auto res_io = Cell_N_io{ OutList(), OutList(), OutList(), OutList(), OutList(), OutList(), InpList() };
    auto scratch = CellScratch{
        ...
        make_shape_var_view( tensor_view<CpuHostMemorySpace>( ffi_nb_threads->typed_data(), tuple(  ), tuple(  ) ), SI( max_ffi_nb_threads ), errors, SI( 2 ) ),
        make_shape_var_view( tensor_view<CpuHostMemorySpace>( ffi_nb_words->typed_data(), tuple(  ), tuple(  ) ), SI( max_ffi_nb_words ), errors, SI( 3 ) ),
        Ct<SI, 32>{}
    };
    auto scratch_io = CellScratch_io{ OutList(), OutList(), OutList(), InpList() };
    errors.fill_with( queue, 0 );
    res.vertex_positions.fill_with( queue, 0 );
    res.vertex_cuts.fill_with( queue, 0 );
    res.vertex_nbrs.fill_with( queue, 0 );
    res.cut_ids.fill_with( queue, 0 );
    res.nb_vertices.fill_with( queue, 0 );
    res.nb_cuts.fill_with( queue, 0 );
...
run_parallel(
    ...
    cut_fwd_kernel{},
    InpList(), cut_id, InpList(), direction, InpList(), offset, cell_io, cell, res_io, res, scratch_io, scratch
);
    }
