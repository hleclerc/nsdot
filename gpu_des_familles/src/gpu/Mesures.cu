...
DiagrammeGpu<D,TK>::~DiagrammeGpu() {
    ...
    cudaFree( m.w ); cudaFree( m.ids ); cudaFree( m.res ); cudaFree( m.deb );
    delete impl;
}

namespace {

/// LE LANCEMENT d'un noyau deja choisi : `lance( grid, block )`.
...
Chrono chrono( const Impl &m, int reps, std::vector<double> &res, auto &&lance ) {
    ...
    CUDA_OK( cudaMemcpy( &ch.deborde, m.deb, sizeof( int ), cudaMemcpyDeviceToHost ) );
    ch.retour = now() - t0;
...
Chrono lance2( const Impl &m, Variante v, int reps, std::vector<double> &res ) {
    ...
    if ( v != Variante::FIL ) {
        auto voies = [ & ]( auto vv ) {
            ...
            return chrono<2,TK>( m, reps, res, [ & ]() { noyau2_voies<POIDS,MaxNb,V><<<grid, BLOC2>>>( ar, m.res, m.deb ); } );
        };
    ...
    return chrono<2,TK>( m, reps, res, [ & ]() { noyau2_fil<POIDS,MaxNb><<<grid, bloc>>>( ar, m.res, m.deb ); } );
}
...
Chrono lance3( const Impl &m, Variante v, int reps, std::vector<double> &res ) {
    ...
    const int bloc = 128, grid = ( m.n + bloc - 1 ) / bloc;
    if ( v != Variante::FIL ) {
        std::fprintf( stderr, "3D : la variante 'voies' n'existe pas encore\n" );
        return Chrono{ 0, 0, m.n };
    }
    return chrono<3,TK>( m, reps, res, [ & ]() { noyau3_fil<POIDS,MaxNv,MaxNv><<<grid, bloc>>>( ar, m.res, m.deb ); } );
}
