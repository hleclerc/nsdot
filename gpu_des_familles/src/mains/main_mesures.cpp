...
int main( int argc, char **argv ) {
    ...
    for ( int i = 1; i < argc; ++i ) {
        ...
        Args::usage();
        std::printf( "  --variante V    fil | filreg | filregc | filmix{4,6,8,12,16} | filbrk{6,8,10,12,16} | filbrk8nu | filrot{6,8} | voies | voies16 | voies32 | paquet{8,32}x{1,2,4}[S] | toutes (toutes)\n"
