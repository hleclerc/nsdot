...
UTP HD DTP::Vector( FillWith, auto &&...ctor_args ) {
    for( auto &v : *this )
        new ( &v ) T( ctor_args... );
}

UTP HD DTP::Vector( Function, auto &&func ) {
    for( PI i = 0; i < ct_size; ++i )
        new ( &operator[]( i ) ) T( func( i ) );
}

UTP HD DTP::Vector( Reserved ) {
}
...
UTP HD DTP::Vector( const Vector &that ) : Vector( Reserved() ) {
    for ( PI i = 0; i < that.size(); ++i )
        new ( data() + i ) T( that[ i ] );
}

UTP HD DTP::Vector( Vector &&that ) noexcept {
    for ( PI i = 0; i < PI( ct_size ); ++i )
        new ( data() + i ) T( std::move( that[ i ] ) );
}

UTP HD Vector<T,ct_size>& DTP::operator=( const Vector &that ) {
    if ( this != &that )
        for ( PI i = 0; i < size(); ++i )
            operator[]( i ) = that[ i ];
    return *this;
...
UTP HD DTP& DTP::operator=( Vector &&that ) noexcept {
    if ( this != &that )
        for ( PI i = 0; i < PI( ct_size ); ++i )
...
UTP HD const T& DTP::operator[]( PI index ) const {
    return data()[ index ];
}

UTP HD T& DTP::operator[]( PI index ) {
    return data()[ index ];
}
...
UTP HD DTP DTP::with_value_at( PI index, T value ) {
    return with_func( [=] ( PI i ) { return i == index ? value : T( 0 ); } );
}

UTP HD auto DTP::with_pushed_value( T value ) const {
...
UTP HD auto DTP::without_index( PI ind_to_remove ) const {
    Vector<T,ct_size-1> res( Reserved{} );
