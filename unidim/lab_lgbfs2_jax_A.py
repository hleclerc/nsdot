...
def loss(points,normals, bin_edges,bin_mass,
    ...
    normals_chunks = normals[:n_full].reshape(n_full // C, C, normals.shape[1])
    mass_chunks = bin_mass[:n_full].reshape(n_full // C, C, bin_mass.shape[1])

    def scan_body(value, xs):
        normals_chunk, mass_chunk = xs
        value = value + _loss_chunk(points,normals_chunk,bin_edges,mass_chunk, use_checkpoint=use_checkpoint,)
        return value, None
...
def optimize(points, sino,max_iter=50,
    ...
    history.append({"iteration": 1, "loss": float(wv), "grad_norm": float(jnp.linalg.norm(wg)),
